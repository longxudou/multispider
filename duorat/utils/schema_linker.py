...
from duorat.utils.tokenization import AbstractTokenizer, StanzaTokenizer


class AbstractSchemaLinker(metaclass=abc.ABCMeta):
    @abc.abstractmethod
    def question_to_slml(self, question: str, sql_schema: SQLSchema, lang: str = 'en') -> str:
        pass
...
class SpiderSchemaLinker(AbstractSchemaLinker):
    ...
    def question_to_slml(self, question: str, sql_schema: SQLSchema, lang: str = 'en') -> str:
        if  isinstance(self.tokenizer, StanzaTokenizer) or \
            isinstance(self.tokenizer, CoreNLPTokenizer):
            _tokenize = partial(self.tokenizer.tokenize, lang=lang)
            _lemma = partial(self.tokenizer.lemma, lang=lang)
            _tokenized_question = self.tokenizer.tokenize_with_raw(question, lang=lang)
            _detokenize = partial(self.tokenizer.detokenize, lang=lang)        
        else:
            _tokenize = self.tokenizer.tokenize
            _tokenized_question = self.tokenizer.tokenize_with_raw(question)
            _detokenize = self.tokenizer.detokenize
        
        tagged_question_tokens = tag_question_with_schema_links(
            tokenized_question=_tokenized_question,
        ...
        slml_builder = SLMLBuilder(
            sql_schema=sql_schema, detokenize=_detokenize
        )
        slml_builder.add_question_tokens(question_tokens=tagged_question_tokens)
        slml_question, mentioned_schema_in_sl = slml_builder.build()

        return slml_question, mentioned_schema_in_sl
...
def tag_question_with_schema_links(
    ...
    lemma: Callable[[str], Sequence[str]],
    max_n_gram: int,
    with_stemming: bool,
    blocking_match: bool,
...
def span_matches_entity(
    ...
    if with_stemming:
        ...
        entity_name_seq = pre_process_words(
            tokenize(entity_name), with_stemming=with_stemming, lemma=lemma
        )
