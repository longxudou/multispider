...
class SLMLBuilder(object):
    ...
    def build(self) -> str:
                ...
                for (start, end, match_tag) in sorted(_spans, key=lambda t: t[2]):
                    if idx == start:
                        if isinstance(match_tag, TableMatchTag):
                            ...
                            mentioned_schema.append(('table',match_tag.table_id))
                            ...
                            mentioned_schema.append(('column', match_tag.column_id))
                            if match_tag.confidence == HighConfidenceMatch():
                                confidence = "high"
                            elif match_tag.confidence == LowConfidenceMatch():
                                confidence = "low"
        ...
        return self._detokenize(slml_question), mentioned_schema
