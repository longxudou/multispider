...
from duorat.utils.tokenization import AbstractTokenizer, BERTTokenizer, GRAPPATokenizer, XLMRTokenizer
...
class DuoRATPreproc(abstract_preproc.AbstractPreproc):
    def __init__(self, **kwargs) -> None:
        ...
        self.target_vocab_path = kwargs["target_vocab_pkl_path"]
        self.target_vocab = None

        self.counted_db_ids: Set[int] = set()
        # self.sql_schemas: Dict[str, SQLSchema] = {}
        self.sql_schemas_multi: Dict[str, Dict[str, SQLSchema]] = defaultdict(dict)
        ...
        self.grammar = self.transition_system.spider_grammar

        self.preproc_items: Dict[str, List[RATPreprocItem]] = {}

        self.lang_dict = kwargs.get('langs', dict())
    ...
    def validate_item(
        ...
        lang = self.lang_dict.get(section, 'default')
        if item.spider_schema.db_id not in self.sql_schemas_multi[lang]:
            schema = preprocess_schema_uncached(
                schema=item.spider_schema,
                db_path=item.db_path,
                tokenize=self._schema_tokenize,
            )
            self.sql_schemas_multi[self.lang_dict[section]][item.spider_schema.db_id] = schema
    ...
    def add_item(
        ...
        lang = self.lang_dict.get(section, 'default')
        sql_schema = self.sql_schemas_multi[lang][item.spider_schema.db_id]
        preproc_item_no_shuffle = self.preprocess_item(
            item, sql_schema, validation_info, section
        )
...
class TransformerDuoRATPreproc(DuoRATPreproc):
    ...
    def preprocess_item(
        ...
        slml_question: str = self.schema_linker.question_to_slml(
            ...
            lang=self.lang_dict.get(section, 'en')
...
class BertDuoRATPreproc(DuoRATPreproc):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        assert isinstance(self.tokenizer, BERTTokenizer) or isinstance(self.tokenizer, GRAPPATokenizer)
    ...
    def preprocess_item(
        ...
        slml_question, mentioned_schema_in_sl  = self.schema_linker.question_to_slml(
            question=item.question, 
            sql_schema=sql_schema,
            lang=self.lang_dict.get(section, 'en')
        ) if item.slml_question is None else item.slml_question
        ...
        ##score the schema linking performance

        #a)count the mentioned schema in SQL
        mentioned_schema_in_sql = set(item.orig['mentioned_schema'])

        #b)count the mentioned schema in Schema Linking process
        mentioned_schema_in_sl = set([(i[0], int(i[1])) for i in mentioned_schema_in_sl])

        #c)compute the metric
        #exclude the '*'
        if ('column',0) in mentioned_schema_in_sql:
            mentioned_schema_in_sql.remove(('column',0))
...
@registry.register("preproc", "GRAPPADuoRAT")
class GRAPPADuoRATPreproc(DuoRATPreproc):
    tokenizer: GRAPPATokenizer

    def __init__(self, **kwargs) -> None:
        super(GRAPPADuoRATPreproc, self).__init__(**kwargs)
        self.add_cls_token = kwargs["add_cls_token"]
        self.add_sep_token = kwargs["add_sep_token"]
        assert isinstance(self.tokenizer, GRAPPATokenizer) or\
               isinstance(self.tokenizer, XLMRTokenizer)
    ...
    def preprocess_item(
        ...
        if lang in ['zh', 'ja']:
            ...
            question = []
            for i, q in enumerate(_question):
                if q.value.startswith('▁') and i > 0 and _question[i-1].value != '<s>':
                    if q.value != '▁':
                        q = PreprocQuestionToken(
                            key=q.key,
                            value=q.value[1:],
                            raw_value=q.raw_value,
                            match_tags=q.match_tags
                        )
                        question.append(q)
                else:
                    question.append(q)
    ...
    def save(self) -> None:
        super(GRAPPADuoRATPreproc, self).save()

    def load(self) -> None:
        super(GRAPPADuoRATPreproc, self).load()
